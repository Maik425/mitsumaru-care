...
import { ArrowLeft, Plus, Edit, Trash2, Copy, FileText, Grid3X3, Save, X, Clock, Minus } from "lucide-react"
...
export function RoleTemplatesManagement() {
  ...
  const positions = ["フロアリーダー", "1階担当", "2階担当", "入浴介助", "食事介助", "記録業務", "巡回担当"]
  const shiftTypes = ["早番", "日勤", "遅番", "夜勤", "準夜勤", "深夜勤"]
  const professionTypes = ["介護職", "看護職", "リハビリ職", "栄養士", "相談員", "事務職", "管理職"]
  const areas = ["1階", "2階", "3階", "デイサービス", "ショートステイ", "事務所"]
  const roleOptions = [
    "食事介助",
    "排泄介助",
    "入浴介助",
  ...
  const updateStaffRow = (staffId: number, field: string, value: string) => {
    setVisualTemplate({
      ...visualTemplate,
      staffRows: visualTemplate.staffRows.map((row) => (row.id === staffId ? { ...row, [field]: value } : row)),
    })
  }

  const handleCellClick = (staffId: number, timeIndex: number) => {
    setSelectedCell({ staffId, timeIndex })
    const currentRole = visualTemplate.staffRows.find((row) => row.id === staffId)?.schedule[timeIndex] || ""
    setEditingRole(currentRole)
  }

  const updateScheduleCell = (staffId: number, timeIndex: number, role: string) => {
    setVisualTemplate({
      ...visualTemplate,
  ...
  const saveVisualTemplate = () => {
    ...
    visualTemplate.staffRows.forEach((row) => {
      row.schedule.forEach((role) => {
        if (role) {
          const existingRole = roles.find((r) => r.position === role && r.assignee === row.position)
          if (existingRole) {
              ...
              {areas.map((area) => (
                <button
                  ...
                  className={`w-full text-left px-2 py-1 text-xs rounded hover:bg-gray-100 ${
                    staff.area === area && staff.area !== "その他" ? "bg-blue-100 text-blue-800" : ""
                  }`}
                >
                  {area}
                </button>
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-16">
            ...
            <h1 className="text-xl font-semibold text-gray-900 ml-4">役割表テンプレート登録</h1>
          </div>
        </div>
      </header>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setVisualTemplate({ ...visualTemplate, name: e.target.value })}
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setVisualTemplate({ ...visualTemplate, shift: value })}
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setVisualTemplate({ ...visualTemplate, description: e.target.value })}
              ...
              <div className="border rounded-lg p-4 bg-blue-50">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <Label className="text-sm font-medium">時間スロット管理:</Label>
                    <span className="text-sm text-gray-600">{timeSlots.length}個</span>
                  </div>
                ...
                <div className="grid grid-cols-6 gap-2">
                  {timeSlots.map((time, index) => (
                    <div key={index} className="flex items-center space-x-1 p-2 bg-white rounded border">
                      {editingTimeIndex === index ? (
                        <div className="flex items-center space-x-1 w-full">
                          <Input
                            ...
                            autoFocus
                          />
                          <Button size="sm" variant="ghost" onClick={saveTimeEdit} className="h-6 w-6 p-0">
                            <Save className="h-3 w-3 text-green-600" />
                          </Button>
                          <Button size="sm" variant="ghost" onClick={cancelTimeEdit} className="h-6 w-6 p-0">
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-gray-600">{visualTemplate.staffRows.length}行</span>
                </div>
                <div className="flex space-x-2">
                ...
                <table className="w-full border-collapse border border-gray-400">
                  <thead>
                    <tr className="bg-gray-100">
                      <th className="border border-gray-400 p-2 text-sm font-medium w-16 bg-gray-200">シフト</th>
                      <th className="border border-gray-400 p-2 text-sm font-medium w-24 bg-gray-200">名前</th>
                      <th className="border border-gray-400 p-2 text-sm font-medium w-20 bg-gray-200">職種</th>
                      <th className="border border-gray-400 p-2 text-sm font-medium w-20 bg-gray-200">エリア</th>
                      {timeSlots.map((time) => (
                        <th
                          key={time}
                      ...
                      <th className="border border-gray-400 p-1 text-xs font-medium bg-gray-200 w-10">操作</th>
                    </tr>
                  </thead>
                  <tbody>
                    {visualTemplate.staffRows.map((staff) => (
                      <tr key={staff.id} className="hover:bg-gray-50">
                        <td className={`border border-gray-400 p-2 text-sm ${getShiftBackgroundColor(staff.workType)}`}>
                          <Select
                            value={staff.workType}
                            onValueChange={(value) => updateStaffRow(staff.id, "workType", value)}
                          >
                        ...
                        <td className={`border border-gray-400 p-2 text-sm ${getShiftBackgroundColor(staff.workType)}`}>
                          <Input
                            value={staff.name}
                            onChange={(e) => updateStaffRow(staff.id, "name", e.target.value)}
                        ...
                        <td className={`border border-gray-400 p-2 text-sm ${getShiftBackgroundColor(staff.workType)}`}>
                          <Select
                            ...
                            onValueChange={(value) => updateStaffRow(staff.id, "position", value)}
                          >
                            <SelectTrigger className="h-8 text-xs">
                              <SelectValue />
                            </SelectTrigger>
                        ...
                        <td className={`border border-gray-400 p-1 text-sm ${getShiftBackgroundColor(staff.workType)}`}>
                        ...
                        {staff.schedule.map((task, timeIndex) => (
                          ...
                          >
                            <div className="min-h-[20px] flex items-center justify-center">{task || ""}</div>
                          </td>
                        ))}
                        <td
              ...
              <div className="border border-gray-400 p-4">
                ...
                <Textarea
                  ...
                  onChange={(e) => setVisualTemplate({ ...visualTemplate, handoverNotes: e.target.value })}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-2">
                      <Select
                        ...
                        onValueChange={(value) => setNewRole({ ...newRole, position: value })}
                      >
                        <SelectTrigger>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setNewRole({ ...newRole, assignee: value })}
                      >
                        <SelectTrigger>
                      ...
                      <div className="space-y-2 mt-2">
                        {formData.roles.map((role, index) => (
                          <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                            <div className="flex items-center space-x-2">
                              <Badge className="bg-blue-100 text-blue-800">{role.position}</Badge>
                              <Badge variant="outline">{role.assignee}</Badge>
                              <span className="text-sm text-gray-600">{role.count}人</span>
                            </div>
                            <Button size="sm" variant="ghost" onClick={() => removeRole(index)}>
                ...
                <div className="space-y-4">
                  {templates.map((template) => (
                    <div key={template.id} className="p-4 border border-gray-200 rounded-lg">
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            <Badge className={getShiftColor(template.shift)}>{template.shift}</Badge>
                            <span className="text-sm text-gray-600">使用回数: {template.usageCount}回</span>
                          </div>
                        </div>
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-700 mb-2">役割構成</p>
                          ...
                          {template.roles.map((role, index) => (
                            <div key={index} className="flex items-center justify-between text-sm">
                              <div className="flex items-center space-x-2">
                                ...
                                <span className="text-gray-600">{role.assignee}</span>
                              </div>
                              <span className="text-gray-600">{role.count}人</span>
                            </div>
                          ))}
                        </div>
                      </div>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <div className="text-sm text-gray-600">
                {selectedCell && (
                  <>
                    職員: {visualTemplate.staffRows.find((row) => row.id === selectedCell.staffId)?.name} | 時間:{" "}
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end space-x-2">
                ...
                <Button
                  onClick={() => {
                    if (selectedCell) {
                      updateScheduleCell(selectedCell.staffId, selectedCell.timeIndex, editingRole)
                    }
                  }}
                >
                  設定
