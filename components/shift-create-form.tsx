...
import { Calendar, ArrowLeft, Download, Share2, Save, Eye, ChevronLeft, ChevronRight, Printer } from "lucide-react"
...
export function ShiftCreateForm() {
  ...
  const [viewMode, setViewMode] = useState<"current" | "past" | "future">("current")
  ...
  const shiftTypes = useMemo(
    () => [
      { code: "早", name: "早番", time: "8:30-17:30", color: "bg-blue-100 text-blue-800", hours: 8 },
      { code: "日", name: "日勤", time: "9:00-18:00", color: "bg-green-100 text-green-800", hours: 8 },
      { code: "遅", name: "遅番", time: "10:30-19:30", color: "bg-orange-100 text-orange-800", hours: 8 },
      { code: "休", name: "公休", time: "", color: "bg-gray-100 text-gray-800", hours: 0 },
      { code: "有", name: "有給", time: "", color: "bg-purple-100 text-purple-800", hours: 0 },
    ],
    [],
  )
  ...
  const getQualificationBadge = useCallback((qualification: string) => {
    ...
    return (
      <Badge className={colors[qualification as keyof typeof colors] || "bg-gray-100 text-gray-800"}>
        {qualification}
      </Badge>
    )
  }, [])

  const getCellKey = useCallback((staffId: number, date: number) => `${staffId}-${date}`, [])
    ...
    staff.forEach((person) => {
      for (let date = 1; date <= daysInMonth; date++) {
        ...
        // 土曜日は休みの確率を高める（完全週休2日制）
          ...
          newShifts[key] = shiftOptions[Math.floor(Math.random() * shiftOptions.length)]
        }
      }
    })
  ...
  const getDailyRequirement = useCallback(
    (day: number, shiftType: keyof typeof staffRequirements) => {
      ...
      return dailyStaffRequirements[dayStr]?.[shiftType] || staffRequirements[shiftType]
    },
  ...
  const ShiftCell = useCallback(
    ({ staffId, date }: { staffId: number; date: number }) => {
      ...
      if (isEditing && !isReadOnly) {
        return (
          <div className={`border-r ${isWeekend ? "bg-red-50" : ""}`}>
            <Select value={displayValue} onValueChange={(newValue) => handleShiftChange(staffId, date, newValue)}>
      ...
      return (
        ...
        >
          <Badge className={isSunday ? "bg-gray-100 text-gray-800" : shiftType?.color || "bg-gray-100 text-gray-800"}>
            {displayValue}
          </Badge>
        </div>
      )
    },
    [editingCell, getCellKey, getShiftValue, handleCellClick, handleShiftChange, shiftTypes, year, month, viewMode],
  )
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              ...
              <h1 className="text-xl font-semibold text-gray-900 ml-4">シフト詳細設定</h1>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <Button variant="outline" size="sm" onClick={shareWithStaff} disabled={isSharing}>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label>表示モード</Label>
                  <Select value={viewMode} onValueChange={(value: "current" | "past" | "future") => setViewMode(value)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <DialogContent className="max-w-2xl">
                    ...
                    <div className="space-y-6">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label className="text-sm font-medium">イベント種別</Label>
                          <Select
                            value={newEvent.type}
                            onValueChange={(value: "meeting" | "business_trip" | "training" | "other") =>
                        ...
                        <div>
                          <Label className="text-sm font-medium">配置ルール</Label>
                          <Select
                            value={newEvent.rule}
                            onValueChange={(value: "increase_regular" | "exclude_from_floor" | "custom") =>
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="increase_regular">常勤職員増員</SelectItem>
                              <SelectItem value="exclude_from_floor">現場配置除外</SelectItem>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setNewEvent((prev) => ({ ...prev, startDate: Number.parseInt(e.target.value) || 1 }))
                            }
                            className="mt-1"
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setNewEvent((prev) => ({ ...prev, endDate: Number.parseInt(e.target.value) || 1 }))
                            }
                            className="mt-1"
                          />
                        </div>
                      ...
                      {newEvent.rule === "exclude_from_floor" && (
                        <div>
                          <Label className="text-sm font-medium">対象職員</Label>
                          <Select
                            onValueChange={(value) => {
                                ...
                                <Badge key={name} variant="outline" className="text-sm">
                                  ...
                                  <button
                                    onClick={() =>
                                      setNewEvent((prev) => ({
                                        ...prev,
                                        targetStaff: prev.targetStaff.filter((n) => n !== name),
                                      }))
                                    }
            ...
            <CardContent>
              <div className="space-y-3 max-h-40 overflow-y-auto">
                {specialEvents.length === 0 ? (
                  <div className="text-sm text-gray-500 text-center py-4">登録されたイベントはありません</div>
                ) : (
                  specialEvents.map((event) => (
                    <div key={event.id} className="border rounded p-3 bg-white">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                          onClick={() => setSpecialEvents((prev) => prev.filter((e) => e.id !== event.id))}
                      ...
                      <div className="text-xs text-gray-600 space-y-1">
                        ...
                        {event.description && (
                          <div className="mt-2 p-2 bg-gray-50 rounded text-xs">{event.description}</div>
                        )}
                      </div>
                    </div>
                  ))
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-medium">
                    {viewMode === "past" ? "過去分表示" : viewMode === "future" ? "来月分表示" : "現在編集中"}
                  </span>
                </div>
        ...
        <div className="bg-white rounded-lg shadow overflow-x-auto">
          <div className="min-w-[1200px]">
            ...
            <div className="grid grid-cols-[200px_repeat(31,40px)] gap-0 border-b-2 border-gray-300">
              <div className="p-2 bg-gray-100 border-r font-semibold text-center">職種・氏名</div>
              {Array.from({ length: daysInMonth }, (_, i) => {
                ...
                const hasCustomSetting = dailyStaffRequirements[(i + 1).toString()]
                ...
                return (
                  ...
                  >
                    ...
                    <div className={isWeekend ? "text-red-600" : ""}>{dayOfWeek}</div>
                    <div
                      className={`font-bold mt-1 ${isSunday ? "text-gray-600" : isShortage ? "text-red-600" : "text-blue-600"}`}
                    >
                      {isSunday ? "休業" : `${0}/${requiredTotal}`} {/* 日曜日は休業表示 */}
                    </div>
            ...
            {Object.entries(groupedStaff).map(([category, categoryStaff]) => (
              <div key={category}>
                ...
                <div className="grid grid-cols-[200px_repeat(31,40px)] gap-0 bg-blue-50 border-b">
                  <div className="p-2 font-semibold text-blue-800 border-r">{category}集計</div>
                  {Array.from({ length: daysInMonth }, (_, i) => (
                    <div key={i} className="p-1 text-xs text-center border-r bg-blue-50">
                      <div className="text-blue-600 font-semibold">
                        {
                          categoryStaff.filter(
                            (staff) =>
                              getShiftValue(staff.id, i + 1) !== "休" && getShiftValue(staff.id, i + 1) !== "有",
                          ).length
                        }
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <Badge className="bg-orange-100 text-orange-800">遅</Badge>
                ...
                <div>
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <Badge className="bg-purple-100 text-purple-800">有</Badge>
