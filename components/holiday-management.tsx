...
export default function HolidayManagement() {
  ...
  const holidayRequests = [
    ...
    },
    {
      ...
      reason: "急な家族の用事のため、どなたかと交換をお願いします。介護業務に支障がないよう配慮いたします。",
    ...
    },
    {
      ...
      reason: "子供の卒業式と謝恩会のため、2日間お休みをいただきたく、交換をお願いします。",
      status: "exchange_pending",
  ...
  const generateExchangeProposals = (request: any) => {
    ...
    const compatibleStaff = staffList.filter((staff) => {
      ...
      const hasCommonSkills = staff.skills.some((skill) => requestingStaff.skills.includes(skill))
    ...
    const proposals = compatibleStaff.slice(0, 3).map((staff, index) => ({
      ...
      proposedExchangeDate: getProposedExchangeDate(request.holidayDates[0], index),
  ...
  const calculateCompatibility = (staff1: any, staff2: any) => {
    const commonSkills = staff1.skills.filter((skill: string) => staff2.skills.includes(skill))
  ...
  const handleApproveExchangeRequest = (requestId: number, proposalId: number) => {
  ...
  const getHolidaysForDate = (date: number) => {
    ...
    return filteredRequests.filter((request) => request.holidayDates.includes(dateString))
  }
  ...
  const pendingRequests = holidayRequests.filter(
    (request) => request.status === "pending" || request.status === "exchange_pending",
  )
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">交換案検討中</p>
                <p className="text-2xl font-bold text-orange-600">
                  {holidayRequests.filter((r) => r.status === "exchange_pending").length}
                </p>
              </div>
            </div>
          </CardContent>
                ...
                <p className="text-2xl font-bold text-blue-600">
                  {
                    pendingRequests.filter(
                      (r) =>
                        r.exchangeDeadline &&
                        new Date(r.exchangeDeadline) <= new Date(Date.now() + 3 * 24 * 60 * 60 * 1000),
                    ).length
                  }
                </p>
              </div>
                ...
                {holidayRequests.map((request, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center">
                      ...
                      <span className="ml-2 px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded">{request.type}</span>
                      ...
                      {request.holidayDates.map((date) => (
                        <span
                          ...
                          className={`px-2 py-1 text-xs rounded ${
                            request.type === "有給" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                          }`}
                        >
              ...
              <div className="grid grid-cols-7 gap-1 text-sm">
                ...
                {Array.from({ length: 31 }, (_, i) => i + 1).map((date) => {
                  ...
                  const requestsForDate = holidayRequests.filter((request) =>
                    request.holidayDates.some((holidayDate) => new Date(holidayDate).getDate() === date),
                  )
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
              <div className="p-3 bg-blue-50 rounded-lg">
                ...
                <div className="text-xl font-bold text-blue-800">
                  {holidayRequests.reduce((sum, request) => sum + request.holidayDates.length, 0)}件
                </div>
              </div>
              <div className="p-3 bg-green-50 rounded-lg">
                ...
                <div className="text-xl font-bold text-green-800">
                  {holidayRequests
                    ...
                    .reduce((sum, request) => sum + request.holidayDates.length, 0)}
                  件
                </div>
              </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" onClick={() => setShowApprovalHistory(!showApprovalHistory)}>
        ...
        <CardContent>
          <div className="space-y-4">
            {pendingRequests
              ...
              .map((request) => (
                <Card key={request.id} className={`border-l-4 ${getUrgencyColor(request.urgency || "low")}`}>
                  <CardContent className="pt-4">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            ...
                            {request.exchangeDeadline && ` 期限: ${request.exchangeDeadline}`}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div>
                        <h4 className="font-medium text-gray-900 mb-2">希望休日</h4>
                        <div className="space-y-1">
                      ...
                      {request.requestType === "exchange" && (
                        <div>
                          <h4 className="font-medium text-gray-900 mb-2">理由</h4>
                          ...
                          <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
                            <DialogHeader>
                              <DialogTitle className="flex items-center">
                                ...
                                交換希望休の承認処理 - {selectedExchangeRequest?.name}
                              </DialogTitle>
                            </DialogHeader>

                            <Tabs defaultValue="proposals" className="w-full">
                              <TabsList className="grid w-full grid-cols-3">
                                <TabsTrigger value="proposals">交換案</TabsTrigger>
                                <TabsTrigger value="approval">承認処理</TabsTrigger>
                                <TabsTrigger value="impact">影響分析</TabsTrigger>
                              </TabsList>

                              <TabsContent value="proposals" className="space-y-4">
                                <div className="p-4 bg-orange-50 rounded-lg">
                                  <h3 className="font-medium text-orange-900 mb-2">申請内容</h3>
                                  <div className="grid grid-cols-2 gap-4 text-sm">
                                    <div>
                                      <span className="text-orange-700">希望日:</span>
                                      <span className="ml-2">{selectedExchangeRequest?.holidayDates.join(", ")}</span>
                                    </div>
                                    <div>
                                      <span className="text-orange-700">理由:</span>
                                      <span className="ml-2">{selectedExchangeRequest?.reason}</span>
                                    </div>
                                  </div>
                                </div>
                                ...
                                {exchangeProposals.length > 0 ? (
                                        ...
                                        <CardContent className="pt-4">
                                          <div className="flex items-center justify-between mb-3">
                                            <div className="flex items-center space-x-3">
                                              ...
                                              <div>
                                                <h4 className="font-medium">{proposal.exchangeStaff.name}</h4>
                                                <p className="text-sm text-gray-600">
                                                  {proposal.exchangeStaff.position} | 適合度: {proposal.compatibility}%
                                                </p>
                                              </div>
                                            </div>
                                          ...
                                          <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                                            <div>
                                              <span className="text-gray-600">交換予定日:</span>
                                              <span className="ml-2 font-medium">
                                                {new Date(proposal.proposedExchangeDate).toLocaleDateString("ja-JP", {
                                                  month: "short",
                                                  day: "numeric",
                                                  weekday: "short",
                                                })}
                                            ...
                                            <div>
                                              <span className="text-gray-600">保有技能:</span>
                                              <span className="ml-2">{proposal.exchangeStaff.skills.join(", ")}</span>
                                            </div>
                                          </div>
                                          ...
                                          <div className="grid grid-cols-4 gap-2 text-xs">
                                            <div className="text-center">
                                              <div className="font-medium">
                                                {proposal.impactAnalysis.serviceQuality}%
                                              </div>
                                              <div className="text-gray-600">品質</div>
                                            </div>
                                            <div className="text-center">
                                              <div className="font-medium">
                                                {proposal.impactAnalysis.staffWorkload}%
                                              </div>
                                              <div className="text-gray-600">負荷</div>
                                            </div>
                                            <div className="text-center">
                                              <div className="font-medium">
                                                {proposal.impactAnalysis.skillCoverage}%
                                              </div>
                                              <div className="text-gray-600">技能</div>
                                            </div>
                                            <div className="text-center">
                                              <div className="font-medium">{proposal.impactAnalysis.continuity}%</div>
                                              <div className="text-gray-600">継続性</div>
                                            </div>
                                          </div>
                                        </CardContent>
                                      </Card>
                                  ...
                                  <div className="text-center py-8">
                                    ...
                                    <p className="text-gray-600">適切な交換案が見つかりませんでした</p>
                                  </div>
                                )}
                              </TabsContent>
                              ...
                              <TabsContent value="approval" className="space-y-4">
                                <Card>
                                  <CardHeader>
                                    <CardTitle className="text-lg">承認処理</CardTitle>
                                  </CardHeader>
                                  <CardContent className="space-y-4">
                                    {selectedProposal ? (
                                      <div className="p-4 bg-blue-50 rounded-lg">
                                        <h4 className="font-medium text-blue-900 mb-2">選択された交換案</h4>
                                        <p className="text-blue-700">
                                          {selectedProposal.exchangeStaff.name}さんとの交換 （適合度:{" "}
                                          {selectedProposal.compatibility}%）
                                        </p>
                                      </div>
                                    ) : (
                                      <div className="p-4 bg-gray-50 rounded-lg">
                                        <p className="text-gray-600">交換案を選択してください</p>
                                      </div>
                                    )}

                                    <div>
                                      <Label htmlFor="approvalNotes">承認メモ（任意）</Label>
                                      <Textarea
                                        id="approvalNotes"
                                        value={approvalNotes}
                                    ...
                                    <div className="flex space-x-2">
                                      <Button
                                        onClick={() =>
                                          selectedProposal &&
                                          handleApproveExchangeRequest(selectedExchangeRequest.id, selectedProposal.id)
                                        }
                                    ...
                                    <div className="border-t pt-4">
                                      <h4 className="font-medium text-gray-900 mb-3">却下する場合</h4>
                                      <div className="space-y-3">
                                        ...
                                        <Button
                                          variant="outline"
                                          onClick={() => handleRejectExchangeRequest(selectedExchangeRequest.id)}
                              ...
                              <TabsContent value="impact" className="space-y-4">
                                <Card>
                                  <CardHeader>
                                    <CardTitle className="text-lg">影響分析レポート</CardTitle>
                                  </CardHeader>
                                  <CardContent>
                                    {selectedProposal ? (
                                      <div className="space-y-4">
                                        <div>
                                          <h4 className="font-medium text-gray-900 mb-3">総合評価</h4>
                                          <div className="space-y-3">
                                            <div>
                                              <div className="flex justify-between mb-1">
                                                <span className="text-sm text-gray-600">サービス品質</span>
                                                <span className="text-sm font-medium">
                                                  {selectedProposal.impactAnalysis.serviceQuality}%
                                                </span>
                                              </div>
                                              <Progress
                                                value={selectedProposal.impactAnalysis.serviceQuality}
                                                className="h-2"
                                              />
                                            </div>
                                            <div>
                                              <div className="flex justify-between mb-1">
                                                <span className="text-sm text-gray-600">技能カバー率</span>
                                                <span className="text-sm font-medium">
                                                  {selectedProposal.impactAnalysis.skillCoverage}%
                                                </span>
                                              </div>
                                              <Progress
                                                value={selectedProposal.impactAnalysis.skillCoverage}
                                                className="h-2"
                                              />
                                            </div>
                                            <div>
                                              <div className="flex justify-between mb-1">
                                                <span className="text-sm text-gray-600">業務継続性</span>
                                                <span className="text-sm font-medium">
                                                  {selectedProposal.impactAnalysis.continuity}%
                                                </span>
                                              </div>
                                              <Progress
                                                value={selectedProposal.impactAnalysis.continuity}
                                                className="h-2"
                                              />
                                            </div>
                                          </div>
                                        ...
                                        <div className="p-4 bg-blue-50 rounded-lg">
                                          ...
                                          <ul className="text-sm text-blue-700 space-y-1">
                                            <li>• 交換実施前に両職員への事前説明を行ってください</li>
                                            <li>• 必要に応じて業務引き継ぎの時間を設けてください</li>
                                            <li>• 交換後のフォローアップを計画してください</li>
                                          </ul>
                                        </div>
                                      </div>
                                    ) : (
                                      <p className="text-gray-600">交換案を選択すると詳細な影響分析が表示されます</p>
                                    )}
                                  </CardContent>
                                </Card>
                              </TabsContent>
                            ...
                            <DialogContent>
                              ...
                              <div className="space-y-4">
                                <div className="p-4 bg-green-50 rounded-lg">
                                  <h4 className="font-medium text-green-900 mb-2">承認内容</h4>
                                  <p className="text-green-700">
                                    {request.name}さんの{request.type}申請を承認します
                                  </p>
                                ...
                                <div className="flex space-x-2">
                                  <Button
                                    onClick={() => handleApproveRegularRequest(request.id)}
                                    className="bg-green-600 hover:bg-green-700"
                                  >
                                    承認する
                                  </Button>
                            ...
                            <DialogContent>
                              ...
                              <div className="space-y-4">
                                <div className="p-4 bg-red-50 rounded-lg">
                                  <h4 className="font-medium text-red-900 mb-2">却下内容</h4>
                                  <p className="text-red-700">
                                    {request.name}さんの{request.type}申請を却下します
                                  </p>
                                </div>
                                <div>
                                ...
                                <Button
                                  onClick={() => handleRejectRegularRequest(request.id)}
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="font-medium">
                        申請ID: {history.requestId} - {history.action === "approved" ? "承認" : "却下"}
                      </p>
                      <p className="text-sm text-gray-600">
  ...
  const [holidays, setHolidays] = useState([
    { date: "2024-03-01", type: "paid_leave", staff_name: "山田 太郎", status: "confirmed" },
    { date: "2024-03-01", type: "public_holiday", staff_name: "建国記念の日", status: "confirmed" },
    { date: "2024-03-05", type: "paid_leave", staff_name: "田中 花子", status: "confirmed" },
    { date: "2024-03-10", type: "paid_leave", staff_name: "鈴木 一郎", status: "confirmed" },
    { date: "2024-03-10", type: "paid_leave", staff_name: "佐藤 健太", status: "confirmed" },
    { date: "2024-03-10", type: "paid_leave", staff_name: "小林 愛", status: "confirmed" },
    { date: "2024-03-12", type: "paid_leave", staff_name: "渡辺 美咲", status: "confirmed" },
    { date: "2024-03-15", type: "paid_leave", staff_name: "加藤 優子", status: "confirmed" },
    { date: "2024-03-20", type: "paid_leave", staff_name: "高橋 次郎", status: "confirmed" },
    { date: "2024-03-22", type: "paid_leave", staff_name: "伊藤 誠", status: "confirmed" },
    { date: "2024-03-25", type: "paid_leave", staff_name: "中村 静香", status: "confirmed" },
    { date: "2024-03-28", type: "paid_leave", staff_name: "木村 拓也", status: "confirmed" },
    { date: "2024-03-30", type: "paid_leave", staff_name: "斎藤 明", status: "confirmed" },
  ])
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-16">
            ...
            <h1 className="text-xl font-semibold text-gray-900 ml-4">休み管理</h1>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-1 block">対象月</label>
                    ...
                    <div>
                      ...
                      <Select value={filterStatus} onValueChange={setFilterStatus}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="exchange_pending">交換案検討中</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
            ...
            <Card className="border-green-200 bg-green-50">
              <CardContent className="pt-6">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h3 className="font-medium text-green-900 mb-1">承認管理ダッシュボード</h3>
            ...
            <Card className="border-blue-200 bg-blue-50">
              <CardContent className="pt-6">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h3 className="font-medium text-blue-900 mb-1">休暇割り振りモード</h3>
                ...
                <div className="grid grid-cols-7 gap-1">
                  {calendarDays.map((day, index) => {
                    ...
                    const dayRequests = holidayRequests
                      .filter((request) =>
                        request.holidayDates.some((holidayDate) => isSameDay(new Date(holidayDate), day)),
                      )
                      .map((request) => ({
                        date: day.toISOString().split("T")[0],
                        type: request.type === "有給" ? "paid_leave" : "public_holiday",
                      ...
                      >
                        ...
                        <div className="space-y-1">
                          ...
                          {allDayItems.length > 3 && (
                            <div className="text-xs text-gray-500">+{allDayItems.length - 3}件</div>
                          )}
                        </div>
                      </div>
                    )
              ...
              <CardContent className="pt-6">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="text-sm text-gray-600">有給休暇（確定）</span>
                  </div>
                  <div className="flex items-center">
                  ...
                  <div className="border-t pt-2 mt-3">
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="text-sm text-gray-600">有給希望（承認待ち）</span>
                    </div>
                    <div className="flex items-center">
                      <div className="w-4 h-4 bg-red-50 border border-red-200 border-dashed rounded mr-2"></div>
                      <span className="text-sm text-gray-600">公休希望（承認待ち）</span>
                    </div>
                    <div className="flex items-center">
                      <div className="w-2 h-2 bg-orange-400 rounded-full mr-3 ml-1"></div>
                      <span className="text-sm text-gray-600">承認待ちマーク</span>
                    </div>
                  </div>
                </div>
              </CardContent>
        ...
        {filteredRequests.length === 0 && viewMode !== "assign" && viewMode !== "approval" && (
          <Card>
        ...
        {showAssignModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-96 max-w-md mx-4">
              ...
              <div className="mb-4">
                <p className="text-sm text-gray-600 mb-2">選択日: {clickedDate}</p>
              </div>

              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-2">職員選択</label>
                  <select
                    value={modalStaff}
              ...
              <div className="flex space-x-3 mt-6">
                ...
                <Button onClick={handleModalAssign} className="flex-1 bg-blue-600 hover:bg-blue-700">
                  割り振り
                </Button>
              </div>
            </div>
