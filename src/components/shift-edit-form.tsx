...
import { ArrowLeft, Calendar, Zap, Target, Clock, CheckCircle, X, Eye, Save, User } from "lucide-react"
...
export function ShiftEditForm(props: {
  ...
  const { initialTitle = "", initialNotes = "", initialLine = "", onSubmit } = props
  ...
  const [configurationRules, setConfigurationRules] = useState<ConfigurationRule[]>([
    {
      id: "1",
      type: "weekly",
      name: "毎週木曜日増員",
  ...
  const [specificAssignments, setSpecificAssignments] = useState<SpecificStaffAssignment[]>([
    {
      id: "1",
      staffName: "佐藤 寿子",
      date: "2025-03-10",
  ...
  const [editingAssignment, setEditingAssignment] = useState<SpecificStaffAssignment | null>(null)
  const [assignmentFormData, setAssignmentFormData] = useState<Partial<SpecificStaffAssignment>>({
    staffName: "",
    date: "",
    shift: "日勤",
    reason: "",
  ...
  const handleAutoGenerate = async () => {
    ...
    const patterns: ShiftPattern[] = [
      ...
      },
      {
        ...
        score: 78,
        pros: ["全日程で十分な人員確保", "管理者配置が安定", "緊急時対応余裕あり"],
  ...
  const generateSampleShifts = (type: "balanced" | "stable") => {
    ...
    const staff = ["田中 花子", "佐藤 太郎", "山田 次郎", "鈴木 一郎", "高橋 美咲", "伊藤 健太", "渡辺 由美"]
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (type === "balanced") {
        shifts[dateKey] = {
          ...
          日勤: [staff[(day + 2) % 7], staff[(day + 3) % 7], staff[(day + 4) % 7]],
          遅番: [staff[(day + 5) % 7], staff[(day + 6) % 7]],
        }
      } else {
        shifts[dateKey] = {
          早番: [staff[day % 7], staff[(day + 1) % 7], staff[(day + 2) % 7]],
          日勤: [staff[(day + 3) % 7], staff[(day + 4) % 7], staff[(day + 5) % 7], staff[(day + 6) % 7]],
  ...
  const saveConfigurationRule = () => {
    ...
    if (editingRule) {
      setConfigurationRules(configurationRules.map((rule) => (rule.id === editingRule.id ? newRule : rule)))
    } else {
  ...
  const toggleRuleActive = (id: string) => {
    setConfigurationRules(
      configurationRules.map((rule) => (rule.id === id ? { ...rule, isActive: !rule.isActive } : rule)),
    )
  }
  ...
  const handleSaveAssignment = () => {
    if (!assignmentFormData.staffName || !assignmentFormData.date || !assignmentFormData.shift) {
      return
    }
    ...
    if (editingAssignment) {
      setSpecificAssignments((prev) =>
        prev.map((assignment) => (assignment.id === editingAssignment.id ? newAssignment : assignment)),
      )
    } else {
  ...
  const handleDeleteAssignment = (id: string) => {
    setSpecificAssignments((prev) => prev.filter((assignment) => assignment.id !== id))
  }
  ...
  const ShiftTablePreview = ({ pattern }: { pattern: ShiftPattern }) => {
    ...
    const staffData = [
      // 相談員
      { name: "佐○寿○", position: "相談員", employment: "常勤", qualification: "介福", category: "相談員" },
      { name: "茂○五○", position: "相談員", employment: "常勤", qualification: "介福", category: "相談員" },
      { name: "三○涼○", position: "主任", employment: "常勤", qualification: "介福", category: "相談員" },
      ...
      { name: "三○涼○", position: "主任兼介護", employment: "常勤", qualification: "介福", category: "介護職員" },
      {
        name: "清○大○",
        position: "介護",
        employment: "常勤",
      ...
      { name: "沢○亮○", position: "介護", employment: "常勤", qualification: "", category: "介護職員" },
      { name: "梓○咲○", position: "介護", employment: "常勤", qualification: "", category: "介護職員", special: "☆" },
      { name: "桜○麻○", position: "介護", employment: "非常勤", qualification: "介福", category: "介護職員" },
      {
        name: "牧○原○子",
        position: "介護",
        employment: "非常勤",
      ...
      { name: "双○理○", position: "介護", employment: "非常勤", qualification: "", category: "介護職員" },
      { name: "古○朋○", position: "介護", employment: "非常勤", qualification: "", category: "介護職員" },
      { name: "本○茂○", position: "介護", employment: "非常勤", qualification: "", category: "介護職員" },

      // 機能訓練員
      { name: "佐○太○", position: "機能訓練員", employment: "常勤", qualification: "PT", category: "機能訓練員" },
      { name: "山○一○", position: "機能訓練員", employment: "非常勤", qualification: "PT", category: "機能訓練員" },

      // 看護職員
      { name: "健○花○", position: "看護師", employment: "常勤", qualification: "NS", category: "看護職員" },
      { name: "佐○木有○", position: "看護師", employment: "非常勤", qualification: "NS", category: "看護職員" },
      { name: "江○拓○", position: "看護師", employment: "非常勤", qualification: "NS", category: "看護職員" },
    ]
    ...
    const getStaffShiftForDate = (staffName: string, date: string, shifts: any) => {
      ...
      for (const [shiftType, staffList] of Object.entries(shifts)) {
        if (Array.isArray(staffList) && staffList.includes(staffName)) {
          return shiftType === "早番" ? "早" : shiftType === "日勤" ? "日" : "遅"
        }
      }
    ...
    const calculateCategoryTotals = (category: string) => {
      return dates.map(({ date, dayOfWeek }) => {
        ...
        categoryStaff.forEach((staff) => {
          ...
          const isWorking =
            shifts.早番?.includes(staff.name) || shifts.日勤?.includes(staff.name) || shifts.遅番?.includes(staff.name)
    ...
    const categories = [
      ...
      { name: "介護職員", staff: staffData.filter((s) => s.category === "介護職員") },
      { name: "機能訓練員", staff: staffData.filter((s) => s.category === "機能訓練員") },
      { name: "看護職員", staff: staffData.filter((s) => s.category === "看護職員") },
    ]
      ...
      <div className="bg-white p-6 rounded-lg border">
        <div className="mb-6">
          ...
          <div className="text-center text-sm text-gray-600 mb-4">
            早番 8:30~17:30 ☆入浴 L日勤 9:00~18:00 お遅番時間帯責任者連 遅番 10:30~19:30 Pリハビリ補佐
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs border-collapse">
            ...
            <thead>
              <tr>
                <th className="border p-1 bg-gray-100 min-w-[120px]">職種・氏名</th>
                ...
                {dates.map(({ day, dayOfWeek }) => (
                  <th key={day} className="border p-1 bg-gray-100 w-8">
                    <div className="text-center">
                      <div>{day}</div>
                      <div className={dayOfWeek === "土" || dayOfWeek === "日" ? "text-red-600" : ""}>{dayOfWeek}</div>
                    </div>
                  </th>
                ))}
              ...
              {categories.map((category) => (
                <React.Fragment key={category.name}>
                  ...
                  <tr className="bg-blue-50">
                    <td className="border p-1 font-bold">{category.name}集計</td>
                    <td className="border p-1"></td>
                    <td className="border p-1"></td>
                    {calculateCategoryTotals(category.name).map((count, index) => (
                      <td key={index} className="border p-1 text-center font-bold">
                        {count}
                      </td>
                    ))}
                    <td className="border p-1"></td>
                  ...
                  {category.staff.map((staff) => {
                    ...
                    const workDays = monthlyShift.filter((s) => s === "日" || s === "早" || s === "遅").length
                    const holidays = monthlyShift.filter((s) => s === "休").length
                    const paidLeave = monthlyShift.filter((s) => s === "有").length
                      ...
                      <tr key={staff.name}>
                        <td className="border p-1">
                          <div className="flex flex-col">
                            ...
                            <div className="text-xs text-gray-600">{staff.special || ""}</div>
                            <div>{staff.name}</div>
                          </div>
                        </td>
                        <td className="border p-1 text-center">{staff.employment}</td>
                        <td className="border p-1 text-center">{staff.qualification}</td>
        ...
        <div className="mt-4 text-xs text-gray-600">
          ...
          <div className="mt-2 text-right">{new Date().toLocaleDateString("ja-JP")}</div>
        </div>
      </div>
    )
  }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-lg w-full max-w-[95vw] h-full max-h-[95vh] overflow-hidden flex flex-col">
            <div className="p-4 border-b flex justify-between items-center">
              <h2 className="text-xl font-bold">シフト表詳細 - パターン{showDetailedView}</h2>
              <Button variant="outline" onClick={() => setShowDetailedView(null)}>
                <X className="h-4 w-4 mr-2" />
                閉じる
              </Button>
            </div>
            <div className="flex-1 overflow-auto p-4">
              <ShiftTablePreview pattern={generatedPatterns.find((p) => p.id === showDetailedView)!} />
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg w-full max-w-7xl h-full max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b">
              <h2 className="text-2xl font-bold text-gray-900">シフトパターン選択</h2>
              <p className="text-gray-600 mt-2">2つのパターンから最適なシフト表を選択してください</p>
            </div>
                  ...
                  <Card key={pattern.id} className="h-full flex flex-col">
                    ...
                    <CardContent className="flex-1 flex flex-col">
                      ...
                      <div className="flex space-x-2">
                        <Button onClick={() => handlePatternSelect(pattern)} className="flex-1">
                          <Save className="h-4 w-4 mr-2" />
                          このパターンを選択
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => setShowDetailedView(pattern.id)}>
            ...
            <div className="p-6 border-t">
              <Button variant="outline" onClick={() => setShowPatternSelection(false)} className="w-full">
                キャンセル（再生成）
              </Button>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              ...
              <h1 className="text-xl font-semibold text-gray-900 ml-4">シフト簡単作成</h1>
            </div>
            <div className="flex items-center space-x-2">
              {generationComplete && selectedPattern && (
                <div className="flex items-center space-x-2">
                  <Badge className="bg-green-100 text-green-800">{selectedPattern.name} 選択済み</Badge>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="mb-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t">
                  <Button onClick={handleAutoGenerate} disabled={isGenerating} className="w-full" size="lg">
                    {isGenerating ? (
                      <>
          ...
          <Card>
                  ...
                  <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="border rounded p-4 bg-gray-50">
                        <div className="text-sm font-medium mb-3">日付選択（複数選択可）</div>
                        ...
                        <div className="flex items-center justify-between mt-4">
                          <div className="text-sm text-gray-600">{selectedDates.length}日選択中</div>
                          <div className="flex space-x-2">
                      ...
                      {selectedDates.length > 0 && (
                        <div className="border rounded p-4">
                          <div className="text-sm font-medium mb-4">選択日の人数設定 ({selectedDates.length}日)</div>
                          <div className="grid grid-cols-3 gap-6">
                            {Object.entries(basicRequirements).map(([shift, defaultCount]) => {
                              const currentValue =
                                selectedDates.length > 0
                                  ? dailyRequirements[selectedDates[0].toString()]?.[
                              ...
                              return (
                                <div key={shift} className="space-y-2">
                                  <Label className="text-sm font-medium">{shift}番</Label>
                                  <Input
                                    type="number"
                                    min="1"
                                    max="10"
                                      ...
                                      setDailyRequirements((prev) => {
                                        ...
                                        selectedDates.forEach((day) => {
                                          if (!updated[day.toString()]) {
                                            updated[day.toString()] = { ...basicRequirements }
                                          }
                                          updated[day.toString()][shift as keyof typeof basicRequirements] = newValue
                                        })
                                        return updated
                                      })
                                    }}
                      ...
                      <div className="border rounded p-4 max-h-64 overflow-y-auto">
                        <div className="text-sm font-medium mb-3">個別設定済み日付</div>
                        {Object.keys(dailyRequirements).length === 0 ? (
                          <div className="text-sm text-gray-500 text-center py-8">個別設定はありません</div>
                        ) : (
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="text-sm text-gray-600 mb-3">基本テンプレート</div>
                <div className="flex items-center justify-between">
                  <Label>早番</Label>
                  <Input
                    type="number"
                ...
                <Button
                  ...
                  onClick={() => {
                    const newDailyRequirements: Record<string, { 早番: number; 日勤: number; 遅番: number }> = {}
                    for (let day = 1; day <= daysInMonth; day++) {
                      newDailyRequirements[day.toString()] = { ...basicRequirements }
                    }
            ...
            <CardContent>
              <div className="space-y-3">
                {specificAssignments.length === 0 ? (
                  <p className="text-sm text-gray-500 text-center py-4">指定配置はありません</p>
                ) : (
                  specificAssignments.map((assignment) => (
                    <div key={assignment.id} className="border rounded p-3 bg-gray-50">
                      <div className="flex items-center justify-between mb-2">
                        <div className="font-medium text-sm">{assignment.staffName}</div>
                      ...
                      <div className="text-xs text-gray-600 space-y-1">
                        ...
                        {assignment.reason && <div>理由: {assignment.reason}</div>}
                      </div>
                    </div>
                  ))
                )}
                  ...
                  {requestedLeaves.map((leave, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center">
                        ...
                        <span className="ml-2 px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded">{leave.type}</span>
                      </div>
                        ...
                        {leave.dates.map((date) => (
                          <span
                            ...
                            className={`px-2 py-1 text-xs rounded ${
                              leave.type === "有給" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                            }`}
                          >
                            {date}日
                          </span>
              ...
              <div>
                ...
                <div className="grid grid-cols-7 gap-1 text-sm">
                  ...
                  {Array.from({ length: 31 }, (_, i) => i + 1).map((date) => {
                    const dayOfWeek = new Date(selectedYear, selectedMonthNumber - 1, date).getDay()
                    const leavesForDate = requestedLeaves.filter((leave) => leave.dates.includes(date))
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                <div className="p-3 bg-blue-50 rounded-lg">
                  ...
                  <div className="text-xl font-bold text-blue-800">
                    {requestedLeaves.reduce((sum, leave) => sum + leave.dates.length, 0)}件
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold">{editingAssignment ? "指定配置を編集" : "指定配置を追加"}</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setAssignmentFormData((prev) => ({ ...prev, staffName: value }))}
                >
                  <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setAssignmentFormData((prev) => ({ ...prev, reason: e.target.value }))}
                  placeholder="配置理由を入力"
                />
              </div>
            </div>
            ...
            <div className="flex justify-end space-x-2 mt-6">
              ...
              <Button onClick={handleSaveAssignment}>{editingAssignment ? "更新" : "追加"}</Button>
            </div>
          </div>
        </div>
      )}
